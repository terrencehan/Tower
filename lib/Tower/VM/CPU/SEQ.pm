...
sub handle_jXX{
    ...
    given($self->{ifun}){
        ...
        when($FJLE){$self->{cnd}    = ($self->{cc}->{"ZF"} == 1 or $self->{cc}->{"SF"} == 1)}
        when($FJL){$self->{cnd}     = ($self->{cc}->{"ZF"} == 0 and $self->{cc}->{"SF"} == 1)}
        when($FJE){$self->{cnd}     = ($self->{cc}->{"ZF"} == 1)}
        when($FJNE){$self->{cnd}    = ($self->{cc}->{"ZF"} == 0)}
        when($FJGE){$self->{cnd}    = ($self->{cc}->{"ZF"} == 1 or $self->{cc}->{"SF"} == 0)}
        when($FJG){$self->{cnd}     = ($self->{cc}->{"ZF"} == 0 and $self->{cc}->{"SF"} == 0)}
    }
